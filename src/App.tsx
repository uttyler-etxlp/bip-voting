...
const QUESTIONS = [
  {
    id: "q1",
    text: "Would you use or buy this — or know someone who would?",
    weight: 0.30,
    label: "Real demand",
  },
  {
    id: "q2",
    text: "Did the problem feel real and worth solving?",
    weight: 0.25,
    label: "Problem clarity",
  },
  {
    id: "q3",
    text: "Did the solution sound better than what's already out there?",
    weight: 0.25,
    label: "Solution strength",
  },
  {
    id: "q4",
    text: "Overall — how compelling was this pitch?",
    weight: 0.20,
  ...
  return (
    <div className="star-rating">
      ...
      <span className="star-label">
        {hovered || value ? labels[hovered || value] : "Tap to rate"}
      </span>
    </div>
  );
}
...
function PitchCard({ pitch, index, existingVote, onVoteSubmitted, userEmail }) {
  const [ratings, setRatings] = useState(
    existingVote || { q1: 0, q2: 0, q3: 0, q4: 0 }
  );
  ...
  const weightedScore = QUESTIONS.reduce(
    (sum, q) => sum + (ratings[q.id] / 5) * q.weight * 100,
    0
  );
          ...
          {!submitted ? (
            ...
            >
              {submitting ? (
                ...
                `Answer all 4 questions to vote`
              )}
            </button>
          ) : (
      ...
      <div className="progress-bar-wrap">
        <div className="progress-bar-track">
          <div
            ...
            style={{ width: totalPitches ? `${(votedCount / totalPitches) * 100}%` : "0%" }}
          />
        </div>
...
export default function App() {
  const { user, loading: authLoading, signIn, signOut } = useGoogleAuth();
  ...
  // Config — in production these come from the Sheet's config tab
  ...
  async function loadData() {
    ...
    try {
      ...
      // Convert vote array to map keyed by pitchId
      const voteMap = {};
  ...
  if (!user) {
    return (
      <>
        <GlobalStyles />
        <SignInScreen onSignIn={signIn} loading={authLoading} />
      </>
    );
  }
          ...
          <div className="section-header">
            <div>
              ...
              <p className="section-sub">
                {pitches.length} idea{pitches.length !== 1 ? "s" : ""} competing · Score each one to help pick the winners
              </p>
            </div>
    ...
    <style>{`
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      :root {
        ...
        --radius: 12px;
        --radius-sm: 8px;
        --shadow: 0 2px 12px rgba(26,18,6,0.08);
        --shadow-lg: 0 8px 32px rgba(26,18,6,0.12);
      }
      ...
      .signin-screen {
        ...
        background: var(--light);
        padding: 1rem;
        background-image: radial-gradient(circle at 20% 20%, rgba(232,119,34,0.08) 0%, transparent 50%),
                          radial-gradient(circle at 80% 80%, rgba(232,119,34,0.06) 0%, transparent 50%);
      }
      .signin-card {
      ...
      .app-header {
        ...
        box-shadow: 0 1px 8px rgba(26,18,6,0.06);
      }
      .app-header__inner {
        max-width: 760px;
        margin: 0 auto;
      ...
      @media (max-width: 480px) {
        .app-header__inner { flex-wrap: wrap; gap: 8px; }
        .voting-status { font-size: 11px; }
        .pitch-name { font-size: 14px; }
        .voted-badge { display: none; }
      }
    `}</style>
  );
}
